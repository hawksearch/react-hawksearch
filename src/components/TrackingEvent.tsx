...
export const Track = (eventName, args) => {
	switch (eventName.toLowerCase()) {
		case 'pageload':
			...
			return writePageLoad(args.pageType);
		case 'searchtracking':
		...
		case 'bannerimpression':
			...
			return writeBannerImpression(args.bannerId, args.campaignId, args.trackingId); // CHANGED
		case 'sale':
			...
			return writeSale(args.orderNo, args.itemList, args.total, args.subTotal, args.tax, args.currency);
		case 'add2cart':
			// HawkSearch.Tracking.track('add2cart',{uniqueId: '123456789', price: 19.99, quantity: 3, currency: 'USD'});
			return writeAdd2Cart(args.uniqueId, args.price, args.quantity, args.currency);
		case 'add2cartmultiple':
			...
			return writeAdd2CartMultiple(args);
		case 'rate':
			// HawkSearch.Tracking.track('rate', {uniqueId: '123456789',value: 3.00});
			return writeRate(args.uniqueId, args.value);
		case 'recommendationclick':
			...
			return writeRecommendationClick(args.widgetGuid, args.uniqueId, args.itemIndex, args.requestId);
		case 'autocompleteclick':
			...
			return writeAutoCompleteClick(args.keyword, args.suggestType, args.name, args.url); // CHANGED
	}
};

const writePageLoad = pageType => {
...
const writeSearchTracking = (trackingId, typeId) => {
...
const writeClick = (event, uniqueId, trackingId, url) => {
	const c = document.documentElement;
	const pl = {
		EventType: E_T.click,
		EventData: btoa(
...
const writeBannerClick = (bannerId, campaignId, trackingId) => {
	const pl = {
...
const writeBannerImpression = (bannerId, campaignId, trackingId) => {
	const pl = {
...
const writeSale = (orderNo, itemList, total, subTotal, tax, currency) => {
	const pl = {
		EventType: E_T.sale,
		EventData: btoa(
			JSON.stringify({
...
const writeAdd2Cart = (uniqueId, price, quantity, currency) => {
	const pl = {
...
const writeAdd2CartMultiple = args => {
	const pl = {
...
const writeRate = (uniqueId, value) => {
	const pl = {
		EventType: E_T.rate,
		EventData: btoa(
			JSON.stringify({
...
const writeRecommendationClick = (widgetGuid, uniqueId, itemIndex, requestId) => {
	const pl = {
...
const writeAutoCompleteClick = (keyword, suggestType, name, url) => {
	const pl = {
...
const mr = data => {
	...
	fetch('http://tracking-dev.hawksearch.net/api/trackevent', {
		method: 'POST',
